...
async function ProductPage({searchParams: {url}}:Props) {
    ...
    <div className="p-4 lg:p-10 flex flex-col lg:flex-row w-full">
        <div className="hidden lg:inline space-y-4"> 
            {product.images.map((image, i)=> (
                <Image key={image} src={image} alt={product.title+" "+ i} 
                width={90} height={90} className="rounded-sm border"  />
            ))}
        </div>
        <Carousel opts={{loop: true}} 
        className="w-3/5 mb-10 lg:mb-0 lg:w-full self-start 
  ...
  <CarouselNext />
    ...
    <div className="flex-1 border rounded-md p-5 w-full space-y-5">
